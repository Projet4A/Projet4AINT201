...
#a=(data.FaultStatus!='2. NO INTERVENTION REQUIRED')*1
#a.replace([0,1],["No intervention","Intervention"], inplace=True)
#data["Target"] = a

data["Target"] = data.FaultStatus

encodeur = LabelEncoder()
Y=encodeur.fit_transform(data.Target)
X=data.drop(['DateTime_x','Bearing_Location','Date','EquipmentID','SurveyDate','EquipmentName','AnalysisDate','FaultStatus','FaultName','Comment','Location','Analyst','AreaID','Route','EquipmentName','AreaName','Target'],axis=1)
X_train, X_test, Y_train, Y_test = train_test_split(X,Y,test_size=0.8, random_state=42,shuffle=False)

numerical_features = ['DataValue','Units','RPM']
categorical_features = ['EquipmentClassification','Description_Area','Description_Equipment','MptID','Description_MeasPt','Description_APSet','Description_AnalParm']
...
recall_res_rfc = recall_score(y_true_, y_pred_rfc, average='weighted')
precision_res_rfc = precision_score(y_true_, y_pred_rfc, average='weighted')
f1score_res_rfc = f1_score(y_true_, y_pred_rfc, average='weighted')
...
recall_res_svc = recall_score(y_true_, y_pred_svc, average='weighted')
precision_res_svc = precision_score(y_true_, y_pred_svc, average='weighted')
f1score_res_svc = f1_score(y_true_, y_pred_svc, average='weighted')
acc_res_svc = accuracy_score(y_true_, y_pred_svc)

### XBOOST Regressor Classifier (XGBC)
XGBC = make_pipeline(transformer, xgb.XGBClassifier(n_estimators=500,
                                                    max_depth=5,
...
## Entrainement avec RandomSearchCV (Fine-tuning)
XGBC = xgb.XGBClassifier()
#cross_val_score(XGBC,X_train,Y_train,cv=5)

param_grid = {'n_estimators':np.arange(100,300),'max_depth':np.arange(2,6),'subsample' : np.array([0.8,0.9]), 'gamma' : np.array([0.1,1,10]) }

grid = RandomizedSearchCV(XGBC,param_grid, cv=5, n_iter=3)
grid.fit(transformer.fit_transform(X_train),Y_train)
model = grid.best_estimator_

XGBC = make_pipeline(transformer, model)
...
recall_res_xgbc = recall_score(y_true_, y_pred_xgbc, average='weighted')
precision_res_xgbc = precision_score(y_true_, y_pred_xgbc, average='weighted')
f1score_res_xgbc = f1_score(y_true_, y_pred_xgbc, average='weighted')
...
ax.set_title('Confusion matrix of XGBC')
...
file_name = "F://ML_DL//Programs//PA4_ML//Dashboard_P4A//models//xgb_clb_fsall.pkl"
# save
...
recall_res_knc = recall_score(y_true_, y_pred_knc, average='weighted')
precision_res_knc = precision_score(y_true_, y_pred_knc, average='weighted')
f1score_res_knc = f1_score(y_true_, y_pred_knc, average='weighted')
...
recall_res_lr = recall_score(y_true_, y_pred_lr, average='weighted')
precision_res_lr = precision_score(y_true_, y_pred_lr, average='weighted')
f1score_res_lr = f1_score(y_true_, y_pred_lr, average='weighted')
